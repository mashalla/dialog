...
var AscDialog = new Class({
    ...
    // --- extended ----------- Start Edit ---------------------------------  
    // stores the last focus before diaolog opened	
...
.bind(this)
    ...
    add_pop: function(){
        ...
        this.pop.addEvent('keydown', function(e){
            ...
            // --- extended ----------- Start Edit ---------------------------------  
            // adds a new keyevent, the upper one is used twice, so we could get a strange behaviour by using this one	
...
.bind(this));
        ...
        // --- extended ----------- Start Edit ---------------------------------  
        // changed table layout to div layout
        
        
        // add table for pop with border graphics
        this.poptbl = new Element('div', {
            'class': 'grid'
        }).inject(this.pop);
        /*
        ...
        this.popc = this.pop.getElement('div[class=grid]');
...
.bind(this));
    ...
    set_contents: function(msg, cls, width){
        ...
        // --- extended ----------- Start Edit --------------------------------
        // add role 'dialog' and aria-label 'dialog'
        
        if (this.options.classPrefix == 'Modal') {
            this.pop.setProperty('role', 'dialog');
            this.pop.setProperty('aria-label', 'dialog');
        }
    ...
    // 'space' for trigger actions 
    // 'tab' for changing focus
    
    key: function(e){
        if (this.isShowing && (e.key == 'space')) {
            ...
            if (Browser.name == 'ie') {
                ...
                var fire = !cb.dispatchEvent(evt);
            }
            
        }
        if (this.isShowing && (e.key == 'tab') && e.shift) {
            if (e.target == getFirstTabbableChild(this.pop)) {
                getLastTabbableChild(this.pop).focus();
                e.stop();
            }
        }
        
        if (this.isShowing && (e.key == 'tab') && !e.shift) {
            if (e.target == getLastTabbableChild(this.pop)) {
                getFirstTabbableChild(this.pop).focus();
                e.stop();
            }
        }
    },
    ...
    show: function(){
        if (!this.isShowing) {
            ...
            // --- extended ----------- Start Edit --------------------------------   
            // stores the last focus before diaolog opened
            // setTimeout is used to wait until the dialog is build
            
            
            
            this.lastFocus = document.activeElement;
            var newFocus = getFirstTabbableChild(this.pop);
            if (newFocus != null) {
    ...
    hide: function(e){
        if (this.pop.getStyle('opacity') > 0) {
            ...
            // --- extended ----------- Start Edit --------------------------------   
            // restores the last focus before dialog was opened
            if (this.options.classPrefix == 'Modal') {
    ...
    auto_align: function(el, default_align){
        ...
        var popdim = {
            'x': this.pop.offsetWidth,
            'y': this.pop.offsetHeight
        };
        var align = '';
...
var AscTip = new Class({
    ...
    initialize: function(el, msg, cls, options, width){
        ...
        if (typeOf(el) == 'element') {
            ...
            this.pop.setProperty('id', "tt_" + el.getProperty('id'));
            this.pop.setProperty('role', 'tooltip');
            this.pop.setProperty('aria-hidden', 'true');
            el.setProperty('aria-describedby', "tt_" + el.getProperty('id'));
            el.setProperty('aria-required', 'false');
...
.bind(this));
            this.options.addCloseBtn = true;
            
        }
    },
    save_cancel_tbl: function(extra_td, save_str, save_cls){
        if (!typeOf(save_str) || (save_str == '')) {
            save_str = this.options.strs.save;
        }
        if (!typeOf(save_cls) || (save_cls == '')) {
            save_cls = 'i16 save16';
        }
        ...
        var tr = new Element('div').inject(tbl);
        var td2 = new Element('div', {
            'class': 'save-btn-td'
        }).inject(tr);
        ...
        /*  var td3 = new Element('div', {
         'class': 'or'
         }).set('html', this.options.strs.or).inject(tr);*/
        var td4 = new Element('div', {
            'class': 'cancel'
        }).inject(tr);
        
        if (typeOf(extra_td) == 'element') 
            extra_td.inject(tr);
        
        
        /*
         var cancel_a = new Element('a', {
         'href': '#',
         'class': 'cancel'
         }).set('html', this.options.strs.cancel).inject(td4);
         */
        var cancel = this.gen_btn('form cancel', this.options.strs.cancel, '', '#', 'i16 x16').inject(td4);
    ...
    gen_btn: function(btn_class, str, id, href, div_cls){
        ...
        // --- extended ----------- Start Edit --------------------------------
        // generates fill-Element "font", to avoid wrong outputs of the screenreader
        
        var s11 = new Element('font').inject(s1, 'before');
        
        // --- extended ----------- End Edit --------------------------------
        
        var s2 = new Element('span').inject(s1);
        var s3 = new Element('span').inject(s2);
        var d4 = new Element('div', {
            'html': str
        }).inject(s3);
        if (div_cls) {
            d4.addClass(div_cls);
    ...
    modal_contents: function(msg_cls, hdr, msg){
        ...
        // --- extended ----------- Start Edit --------------------------------
        // adds an id for the heading (not for accessibility, only for layout issues)
    ...
    on_show_confirm: function(confirm_fn){
        ...
        if (confirm_btn) {
            confirm_btn.addEvent('click', function(e){
                e = new Event(e).stop();
                if (confirm_fn != null) 
                    confirm_fn.attempt('', this);
            }
.bind(this));
        }
        
        var cancel_btn = this.pop.getElement('div[class=cancel]');
        if (cancel_btn) {
...
.bind(this));
    ...
    // --- extended ----------- Start Edit -------------------------------- 
    // advanced demodialog to present the features of the accessible AscribeDialog version   	
    
    show_demo: function(modal_cls, msg_cls, hdr, msg, confirm_btn_str, confirm_btn_cls, confirm_fn){
        var group = new Element('div', {
            'role': 'group',
            'aria-describedby': 'groupcontent'
        });
        var groupContent = new Element('div', {
            'id': 'groupcontent',
            'html': msg
        }).inject(group);
        var c = this.modal_contents(msg_cls, hdr, group);
        ...
        this.pop.setProperty('role', 'dialog');
        this.pop.setProperty('aria-label', hdr);
        
        if (hdr) {
            var h2Ground = new Element('div',{
				'id':'headingGround'
            });
            h2Ground.wraps(this.pop.getElement('h2'));
            
        }
        
        
        if (modal_cls = 'e') {
            this.pop.getElement('div[class="mi"]').setStyles({
                'padding-left': '0px',
                'margin-left': '0px'
            });
            this.pop.getElement('p[class=" med"]').setStyles({
                'padding-left': '30px',
                'margin-left': '0px'
            });
            this.pop.getElement('div[class="btn-tbl"]').setStyles({
                'padding-left': '30px',
                'margin-left': '0px'
            });
        }
    ...
    show_confirm: function(modal_cls, msg_cls, hdr, msg, confirm_btn_str, confirm_btn_cls, confirm_fn){
        ...
        // --- extended ----------- Start Edit --------------------------------
        // add role 'dialog' and aria-label 'hdr' which is the header message 
...
.bind(this));
        ...
        // --- extended ----------- Start Edit --------------------------------
        // add role 'alertdialog' and aria-label 'hdr' which is the header message   
        
        this.pop.setProperty('role', 'alertdialog');
        this.pop.setProperty('aria-label', hdr);
